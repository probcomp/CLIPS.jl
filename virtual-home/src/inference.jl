...
"""
    literal_command_inference(domain, state, utterance::String)
...
function literal_command_inference(
    ...
    verbose && println("Evaluating logprobs of utterance: \"$utterance\"")
...
"""
    literal_command_inference(domain, state, utterances::Vector{String})

Runs literal listener inference for a sequence of utterances, combining the 
commands that are inferred for each utterance.
"""
function literal_command_inference(
    domain::Domain, state::State, utterances::Vector{String};
    speaker = pddl"(actor)",
    listener = pddl"(helper)",
    verbose::Bool = false,
    p_thresh = 0.99
)
    # Run inference on first utterance
    results = literal_command_inference(domain, state, utterances[1];
                                        speaker, listener, verbose)
    length(utterances) == 1 && return results
    # Filter out commands above cumulative probability threshold
    commands, command_probs, command_scores = results
    accum_probs = cumsum(command_probs)
    keep_idx = findfirst(>=(p_thresh), accum_probs)
    resize!(commands, keep_idx)
    resize!(command_probs, keep_idx)
    resize!(command_scores, keep_idx)
    # Iterate over remaining utterances
    for utt in utterances[2:end]
        results = literal_command_inference(domain, state, utt;
                                            speaker, listener, verbose)
        # Combine new commands with previous commands
        new_commands = ActionCommand[]
        new_probs = Float64[]
        new_scores = Float64[]
        total_prob = 0.0
        for i in eachindex(results.commands)
            cmd = results.commands[i]
            prob = results.probs[i]
            score = results.scores[i]
            for j in eachindex(commands)
                new_cmd = merge(commands[j], cmd)
                new_prob = command_probs[j] * prob
                new_score = command_scores[j] + score
                push!(new_commands, new_cmd)
                push!(new_probs, new_prob)
                push!(new_scores, new_score)
            end
            total_prob += prob
            total_prob >= p_thresh && break
        end
        # Sort commands by posterior probability
        perm = sortperm(new_scores, rev=true)
        commands = new_commands[perm]
        command_probs = new_probs[perm]
        command_scores = new_scores[perm]
    end
    # Return commands and their posterior probabilities
    return (
        commands = commands,
...
function configure_pragmatic_speaker_model(
    ...
    planner = RTHS(heuristic=heuristic, n_iters=n_iters, reuse_paths=false,
...
function pragmatic_goal_inference(
    ...
    modalities = (:utterance, :action),
    verbose = false,
    goal_labels = ["goal$i" for i in 1:n_goals],
    kwargs...
)
