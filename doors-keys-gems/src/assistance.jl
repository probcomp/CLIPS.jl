...
function compute_plan_metrics(
    ...
    speaker = pddl"(human)", listener = pddl"(robot)", verbose::Bool = false
)
    verbose && println("\nComputing plan metrics...")
    _, wait_costs, _, _ =
        extract_action_costs(spec, [speaker, listener])
    mean_wait_cost = mean(wait_costs)
    plan_length = float(length(plan))
    plan_cost = sum(plan) do act
        act == PDDL.no_op && return mean_wait_cost
        return get_action_cost(spec, act)
    end
...
function literal_assistance_naive(
    ...
    max_steps = 100,
    max_plans = 10,
    cmd_planner = AStarPlanner(DoorsKeysMSTHeuristic(), max_nodes=2^16, verbose=true),
    goal_planner = AStarPlanner(DoorsKeysMSTHeuristic(), max_nodes=2^16, verbose=true),
    verbose::Bool = false
)
    ...
    cmd_successes = Bool[]
    goal_successes = Bool[]
    # Filter out impossible commands
    g_commands = filter!(c -> is_command_possible(c, domain, state), g_commands)
    # Limit number of ground commands we plan for
    if length(g_commands) > max_plans
        shuffle!(g_commands)
        g_commands = g_commands[1:max_plans]
    end
    for cmd in g_commands
    ...
    # Handle case where no commands were possible
    if isempty(g_commands)
        cmd_plan = Term[]
    ...
    plan_metrics = compute_plan_metrics(assist_full_plans, true_goal_spec;
...
function literal_assistance_naive(
    ...
    sys_sample_map!(commands, probs, n_samples) do command, prob
        ...
        result = literal_assistance_naive(
            ...
            assist_obj_type; verbose, true_assist_options, kwargs...
        )
...
"""
    pragmatic_assistance_pmdp(
        pf, domain, state, true_goal_spec, assist_obj_type;
        ...
        for (prob, spec, policy) in zip(probs, goal_specs, policies)
            ...
            if plan_end_val == -Inf
                _, wait_costs, _, _ =
                    extract_action_costs(spec, [speaker, listener])
                mean_wait_cost = mean(wait_costs)
                plan_end_val = -mean_wait_cost * remain_steps
            end
